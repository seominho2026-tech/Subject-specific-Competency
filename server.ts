...
// File Upload Endpoint (Single / Multiple up to 5 files per request)
app.post('/api/upload', upload.array('files', 5), (req, res) => {
  try {
    ...
    res.json({ success: true, files: uploadedResults });
  } catch (error: any) {
    console.error('File upload error:', error);
    res.status(500).json({ error: error.message || '파일 업로드 처리 중 오류가 발생했습니다.' });
  }
});

// Gemini AI Setuk (세특) Draft Generation Endpoint
app.post('/api/gemini/generate-setuk', async (req, res) => {
  try {
    ...
    const targetLength = options?.targetLength || '500'; // '500' chars (~1500 bytes) or '350' or '700'
    const customPrompt = options?.customPrompt?.trim() || '';

    // Tone guideline description in Korean
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`EduPortfolio Server running on http://localhost:${PORT}`);
  });
}
